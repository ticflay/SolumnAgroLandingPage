...
import { LAUDOS_AGRONOMICOS } from '@/lib/services-page-content'

const sectionBgs = ['bg-white', 'bg-sand-100'] as const

export default function LaudosAgronomicos() {
  return (
    <section id="laudos-agronomicos">

      {/* Cabeçalho da seção */}
      <div className="bg-white border-t border-t-sand-200 border-b border-b-sand-200 py-10 md:py-14">
        <div className="max-w-[1400px] mx-auto px-6">
          <FadeIn>
            ...
            <h2 className="font-serif font-normal text-green-950 tracking-[-0.02em] leading-[1.2] text-[clamp(1.75rem,4vw,2.5rem)]">
              Documentação técnica com respaldo agronômico
            </h2>
          </FadeIn>
          <FadeIn delay={0.05}>
            <p className="text-sand-500 text-base leading-[1.7] max-w-[520px] mt-4">
              Laudos elaborados por engenheiro agrônomo habilitado, com ART inclusa e agilidade na entrega — para financiamentos, processos judiciais, licenciamentos e produção rural.
            </p>
          </FadeIn>
        </div>
      </div>
      ...
      {/* Sub-serviços */}
      {LAUDOS_AGRONOMICOS.map((laudo, i) => (
        <div
          key={laudo.id}
          id={laudo.id}
